...
namespace ManejoPresupuesto.Servicios
{
    ...
    public class RepositorioCuentas: IRepositorioCuentas
    {
        ...
        {
            using var connection = new SqlConnection(connectionString);
            return await connection.QueryAsync<Cuenta>(@"SELECT Cuentas.Id, Cuentas.Nombre, Cuentas.Balance, TiposCuentas.Nombre as TipoCuenta
                                                            FROM Cuentas
        ...
        {
            using var connection = new SqlConnection(connectionString);
            return await connection.QueryFirstOrDefaultAsync<Cuenta>(@"SELECT Cuentas.Id, Cuentas.Nombre, Cuentas.Balance, Cuentas.Descripcion, Cuentas.TipoCuentaId
                                                                        FROM Cuentas
                                                                        INNER JOIN TiposCuentas
                                                                        on Cuentas.TipoCuentaId = TiposCuentas.Id
                                                                        WHERE TiposCuentas.UsuarioId = @UsuarioId
